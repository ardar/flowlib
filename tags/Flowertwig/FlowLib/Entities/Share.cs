...
namespace FlowLib.Entities
{
    ...
    public class Share : IShare
    {
        ...
        /// </summary>
        public event EventHandler LastModifiedChanged;
        #endregion
        ...
        {
            ErrorOccured = new EventHandler(OnErrorOccured);
            VirtualDirAdded = new EventHandler(OnVirtualDirAdded);
            VirtualDirRemoved = new EventHandler(OnVirtualDirRemoved);
            HashingCompleted = new EventHandler(OnHashingCompleted);
            HashingStarted = new EventHandler(OnHashingStarted);
            LastModifiedChanged = new EventHandler(OnLastModifiedChanged);
            HashContentInfo = new EventHandler(OnHashContentInfo);
            this.name = name;
        }

        void OnHashContentInfo(object sender, DefaultEventArgs e) { }
        void OnLastModifiedChanged(object sender, DefaultEventArgs e)
        {
            lastModifiedTimeStamp = System.DateTime.Now.Ticks;
        }
        void OnHashingStarted(object sender, DefaultEventArgs e) { }
        void OnHashingCompleted(object sender, DefaultEventArgs e) { }
        void OnVirtualDirRemoved(object sender, DefaultEventArgs e) { }
        void OnVirtualDirAdded(object sender, DefaultEventArgs e) { }
        ...
        public virtual bool AddVirtualDir(string virtualdir, string systempath)
        {
            ...
            var vd = new VirtualDir(systempath, virtualdir);
            try
            {
                lock (virtualDirs)
                {
...
#if !COMPACT_FRAMEWORK
                var t = new Thread(OnAddVirtualDir);
                t.Name = "ShareWorker";
                t.IsBackground = true;
                t.Start(vd);
#else
                Thread t = new Thread(new ThreadStart(OnAddVirtualDir));
                t.Name = "ShareCompactWorker";
                t.IsBackground = true;
                t.Start(vd);
#endif
            }
        ...
        protected virtual void OnAddVirtualDir(object ovd)
        {
            ...
            Save();
            VirtualDirAdded(this, new DefaultEventArgs());
            HashContent();
        }
        ...
        protected virtual void AddDir(string systempath, string virtualdir, VirtualDir vd)
        {
            if (System.IO.Directory.Exists(systempath))
            {
                lock (virtualDirs)
                {
                    foreach (KeyValuePair<string, VirtualDir> item in virtualDirs)
                    {
                        ...
                        {
                            ErrorOccured(this, new DefaultEventArgs(Share.ErrorSourceDirectoryDuplicate, systempath));
                            return;
                        }
                    }
                }
                ...
                catch (System.ArgumentException ae)
                {
                    ...
                    ErrorOccured(this, new DefaultEventArgs(ErrorInvalidChars, ae));
                    return;
                }
                catch (System.Security.SecurityException se)
                {
                    // User does not have the required permission. 
                    ErrorOccured(this, new DefaultEventArgs(ErrorSecurity, se));
                    return;
                }
                #endregion
                ...
                catch (System.IO.DirectoryNotFoundException dnfe)
                {
                    ...
                    ErrorOccured(this, new DefaultEventArgs(ErrorSecurity, dnfe));
                }
                catch (System.UnauthorizedAccessException uae)
                {
                    // User does not have the required permission. 
                    ErrorOccured(this, new DefaultEventArgs(ErrorSecurity, uae));
                }
                catch (System.Security.SecurityException se)
                {
                    // User does not have the required permission. 
                ...
                catch (System.UnauthorizedAccessException uae)
                {
                    ...
                    ErrorOccured(this, new DefaultEventArgs(ErrorAccessDeniedBecauseOfIOError, uae));
                }
                #endregion
            }
            else
            {
                ...
                ErrorOccured(this, new DefaultEventArgs(ErrorDirectoryNotFound, new System.IO.DirectoryNotFoundException("Adding virtual dir failed because directory was not found.")));
            }
        }
        ...
        protected virtual void AddContent(ContentInfo contentInfo, VirtualDir vd)
        {
            ...
            try
            {
                ...
                if (alreadyContainsContentInVirtualNames)
                {
                    ...
                    ErrorOccured(this, new DefaultEventArgs(ErrorContentVirtualNameConflict, contentInfo));
                }
                else
                {
        ...
        public virtual bool RemoveVirtualDir(string systempath)
        {
            ...
            if (containsSystemPath)
            {
                ...
                {
                    VirtualDirRemoved(this, new DefaultEventArgs());
                    if (IsHashing)
                        HashContent();
                }
        ...
        public virtual void HashContent()
        {
            ...
            {
                this.isHashing = true;
                HashingStarted(this, new DefaultEventArgs());
                VirtualDir tmp = null;
                    ...
                    if (item.Value != null && !item.Value.IsHashed)
                    {
                        try
                        {
                            ...
                            if (tmp != null)
                            {
                                ...
                                DefaultEventArgs e = new DefaultEventArgs(0, item.Value);
                        ...
                        catch (KeyNotFoundException) { /* Item seem to have been removed */ }
                        catch (System.ArgumentException)
                        {
                            ...
                            ErrorOccured(this, new DefaultEventArgs(Share.ErrorHashDuplicate, item.Value));
                        ...
                        catch (System.Exception e)
                        {
                            ...
                            ErrorOccured(this, new DefaultEventArgs(Share.ErrorUnknownError, e));
                        }
                        finally
                        {
                            if (tmp != null)
                ...
                Save();
                HashingCompleted(this, new DefaultEventArgs());
        ...
        public virtual void Save()
        {
            ...
            LastModifiedChanged(this, new DefaultEventArgs(0, lastModifiedTimeStamp));
            if (directory != null)
                Save(directory);
        }
        /// <summary>
        ...
        public virtual void Save(string dir)
        {
            ...
            // Saves info on disc
            FileOperations.PathExists(directory);
        ...
        public virtual byte[] GetContent(ContentInfo info, long start, long length)
        {
            try
            {
                ...
                if (ContainsContent(ref info) && start >= 0 && (start + length) <= info.Size && length != 0)
                {
                    ...
                    //{
                    var sr = new StreamReader(info.Get(ContentInfo.STORAGEPATH));
                    var reader = new BinaryReader(sr.BaseStream);
            ...
            catch (System.Exception) { /* TODO : This should be changes later to more specific errors like security and io errors */ }
            return null;
        }

        #region OnEvents
        protected void OnErrorOccured(object sender, DefaultEventArgs e)
        {
