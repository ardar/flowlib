...
namespace FlowLib.Connections.Protocols
{
    ...
    public class TransferNmdcProtocol : BaseTransferProtocol, IProtocolTransfer
    {
        ...
        public TransferNmdcProtocol(ITransfer trans, string dir)
        {
            ...
            ChangeDownloadItem = new EventHandler(OnChangeDownloadItem);
            RequestTransfer = new EventHandler(OnRequestTransfer);
        ...
        void OnError(object sender, DefaultEventArgs e) { }
        void OnRequestTransfer(object sender, DefaultEventArgs e) { }
        void OnChangeDownloadItem(object sender, DefaultEventArgs e) { }
...
//#endif
            ...
            {
                DefaultEventArgs e = new DefaultEventArgs((int)TransferErrors.INACTIVITY);
                Error(trans, e);
                if (!e.Handled)
        ...
        public void ParseRaw(byte[] b, int length)
        {
            ...
            if (rawData)
            {
                ...
                if (b != null && length != 0)
                {
                    var conMsg = new BinaryMessage(trans, b, length);
                    // Plugin handling here
                    var e = new DefaultEventArgs(Actions.CommandIncomming, conMsg);
        ...
        public bool GetSegment(bool requestNewDownloadItem)
        {
            if (requestNewDownloadItem)
            {
                ...
                var usrInfo = trans.User;
                if (usrInfo != null)
                {
                    var eArgs = new DefaultEventArgs(0, dwnItem);
                    ...
                    if (trans.DownloadItem != null && trans.CurrentSegment != null && trans.CurrentSegment.Index != -1)
                    {
                        ...
                        if (trans.DownloadItem.ContentInfo.IsFilelist)
                        {
                            ...
                            else if (userSupport != null && userSupport.BZList && mySupport.BZList)
                            {
                                ...
                                trans.DownloadItem.ContentInfo.Set(ContentInfo.FILELIST, BaseFilelist.BZ);
                            }
                            else
                            {
                                trans.Content.Set(ContentInfo.REQUEST, "MyList.DcLst");
                                trans.DownloadItem.ContentInfo.Set(ContentInfo.FILELIST, BaseFilelist.HUFFMAN);
                            }
                        }
        ...
        public void ActOnInMessage(IConMessage conMsg)
        {
            ...
            else if (message is GetBlocks)
            {
                ...
                trans.Content = new ContentInfo(ContentInfo.REQUEST, getblocks.FileName);
                ...
                switch (adcget.Type.ToLower())
                {
                    ...
                    case "tthl":
                        ...
                        if (adcget.Content.StartsWith("TTH/"))
                        {
                            ...
                            {
                                var bytes = Base32.Decode(tmp.Get(ContentInfo.TTHL));
#if !COMPACT_FRAMEWORK
            ...
            {
                DefaultEventArgs e = new DefaultEventArgs((int)TransferErrors.NO_FREE_SLOTS);
                Error(trans, e);
                if (!e.Handled)
                    trans.Disconnect();
            }
            else if (message is Error)
            {
                ...
                DefaultEventArgs e = null;
                // TODO : Add more error messages here.
                switch (error.Message)
                {
                    case "File Not Available":
                        e = new DefaultEventArgs((int)TransferErrors.FILE_NOT_AVAILABLE);
                        break;
                    default:
                        e = new DefaultEventArgs((int)TransferErrors.UNKNOWN, error.Message);
                        break;
                }
                Error(trans, e);
                if (!e.Handled)
            ...
            else if (message is Failed)
            {
                ...
                switch (failed.Message)
                {
                    case "File Not Available":
                        ...
                        break;
                    default:
                        e = new DefaultEventArgs((int)TransferErrors.UNKNOWN, failed.Message);
                        break;
                }
                Error(trans, e);
                if (!e.Handled)
