...
namespace FlowLib.Connections.Protocols
{
    public class UPnPProtocol : IProtocolUPnP
    {
        ...
        {
            this.MessageReceived = new EventHandler(OnMessageReceived);
            this.MessageToSend = new EventHandler(OnMessageToSend);
        ...
        void OnMessageToSend(object sender, DefaultEventArgs e) { }
        void OnMessageReceived(object sender, DefaultEventArgs e) { }
        ...
        public void ActOnInMessage(IConMessage message)
        {
            ...
            if (msg != null)
            {
                ...
                DefaultEventArgs e = new DefaultEventArgs(0, device);
                Update(con, e);
                if (!e.Handled)
                {
                    ...
                    else
                    {
                        ...
                        DefaultEventArgs e2 = new DefaultEventArgs(Actions.UPnPRootDeviceFound, device);
                        Update(con, e2);
                    }
                }
            }
                    ...
                    if (key != null)
                    {
                        try
                        {
                            if (con.RootDevices.ContainsKey(key))
                            {
                                ...
                                DefaultEventArgs e2 = new DefaultEventArgs(Actions.UPnPDeviceDescription, ret);
                                Update(con, e2);
                                if (!e2.Handled)
                                {
                                    ParseDescription(ref device, ret);
                                    Update(con, new DefaultEventArgs(Actions.UPnPDeviceUpdated, device));
                                }
                            }
                        }
