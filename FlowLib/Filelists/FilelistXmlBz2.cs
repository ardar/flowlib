...
namespace FlowLib.Filelists
{
    public class FilelistXmlBz2 : BaseFilelist
    {
        ...
        public override void CreateShare()
        {
            ...
            if (bz2)
            {
                Console.WriteLine("*** Unpacking\t\t" + DateTime.Now.Ticks);
                var tmpStream = new System.IO.MemoryStream();
                Flowertwig.Utils.Compression.Bz2.Decompress(xmlBaseStream, tmpStream);
                tmpStream.Position = 0;
                reader = new XmlTextReader(tmpStream);
                Console.WriteLine("*** Unpacking done\t" + DateTime.Now.Ticks);
            }
            else
            {
                reader = new XmlTextReader(xmlBaseStream);
            }
            const string XML_FILE ="FILE";
            const string XML_DIRECTORY = "DIRECTORY";
            const string XML_FILELISTING = "FILELISTING";
            const string XML_NAME = "Name";
            const string XML_SIZE = "Size";
            const string XML_TTH = "TTH";

            if (reader != null)
            {
                Console.WriteLine("*** Start reading\t" + DateTime.Now.Ticks);
                do
                {
                    string xmlNodeName = reader.Name.ToUpper();
                    switch (reader.NodeType)
                    {
                        case XmlNodeType.None: break; // Ignore none XML data
                        case XmlNodeType.Element:
                            if (reader.HasAttributes)
                            {
                                if (xmlNodeName.Equals(XML_FILE))
                                {
                                    string name = reader.GetAttribute(XML_NAME);
                                    long size = -1;
                                    try
                                    {
                                        size = long.Parse(reader.GetAttribute(XML_SIZE));
                                    }
                                    catch { }
                                    string tth = reader.GetAttribute(XML_TTH);
                                    ContentInfo ci = null;
                                    ...
                                    ci.Set(ContentInfo.VIRTUAL, directoryPath + name);
                                    ci.Set(ContentInfo.NAME, name);
                                    #endregion

                                    AddFile(ci);
                                }
                                else if (xmlNodeName.Equals(XML_DIRECTORY))
                                {
                                    // Add directory
                                    string name = reader.GetAttribute(XML_NAME);
                                ...
                                else if (xmlNodeName.Equals(XML_FILELISTING))
                                {
                        ...
                        case XmlNodeType.EndElement:
                            if (xmlNodeName.Equals(XML_DIRECTORY))
                            {
                                // Remove directory
                                EndDirectory();
                            }
                ...
                Console.WriteLine("*** End reading\t" + DateTime.Now.Ticks);
            }
        }
        ...
        protected override void EndFilelist()
        {
            try
            {
                ...
                writer.Flush();
                #endregion
                FileOperations.PathExists(systemPath);
                ...
                size = Flowertwig.Utils.Compression.Bz2.Compress(xmlBaseStream, baseStream, 512);
                writer.Close();
                #endregion
        ...
        protected  virtual void UpdateDirectoryPath()
        {
            StringBuilder sb = new StringBuilder();
            string[] tmpDirs = dirs.ToArray();
            for (int i = tmpDirs.Length - 1; i >= 0; i--)
            {
                sb.AppendFormat(@"{0}\", tmpDirs[i]);
            }
        ...
        protected override void AddFile(ContentInfo content)
        {
            if (!fromXml)
            {
                ...
                if (content.IsTth)
                {
                    ...
                    writer.WriteValue(content.Get(ContentInfo.TTH));
                    writer.WriteEndAttribute();
                }
                writer.WriteEndElement();
            }
